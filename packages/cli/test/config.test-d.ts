/**
 * Type tests for the `instances`/`onEvent` config surface (docs review gaps
 * 5-6): locks `RpxdConfig` against `RpxdHandlerOptions` so the two can't
...
describe("RpxdConfig instance/observability knobs", () => {
  ...
  it("onEvent matches the handler's event sink", () => {
    expectTypeOf<RpxdConfig["onEvent"]>().toEqualTypeOf<RpxdEventSink | undefined>();
  });
});

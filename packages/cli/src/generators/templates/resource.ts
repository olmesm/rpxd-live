 ...
 */
import type { Scope } from "./scope.ts";${prismaImport}
...
const client = () => {
  if (import.meta.env.SSR) return import("../adapters/db.ts").then((m) => m.db);
...
function pageRoute(spec: ResourceSpec, n: Names): string {
  ...
  .rpc("toggle", (r) =>
    ...
    : "";
  // `setup` (sync) returns the skeleton; `load` fetches the rows on first paint
  // and every URL change (§7). Auth, when protected, lives in `guard` — it runs
  // before `load` on every URL change, so it's the idiom for access control.
  const guardChain = spec.protectedRoute
    ? `
  .guard((_url, ctx) => {
...
import { ${imports}, type ${n.rowType} } from "../domain/${n.domainFile}.ts";
...
export default live("/${spec.plural}")
  .setup(() => ({ rows: [] as ${n.rowType}[] }))${guardChain}
  .load(async (_url, ctx) => {
...
import { scopeFrom } from "../domain/scope.ts";
import { ${n.listFn} } from "../domain/${n.domainFile}.ts";
...
`
    : "";
  // A protected page's guard redirects when signed out, so the session carries a
...
import route from "../routes/${spec.plural}.tsx";
...
import * as ${n.domainFile} from "./${n.domainFile}.ts";
...
`;
  ...
  return `import { describe, expect, it } from "vitest";
import type { Scope } from "./scope.ts";
import { ${n.createFn}, ${n.listFn}, ${n.removeFn} } from "./${n.domainFile}.ts";

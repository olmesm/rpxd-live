 ...
 * must apply the same `instances.*` capacity knobs and `onSecurityEvent`
 * observability hook that `rpxd start` does — the wiring is one spread, but
 * only a booted dev server proves it reaches the running handler. Injected
 * through `configOverride`, the test/embedding seam over `rpxd.config.ts`.
 */
import { afterAll, beforeAll, describe, expect, it } from "bun:test";
import { fileURLToPath } from "node:url";
import type { SecurityEvent } from "@rpxd/server-bun";
...
describe("rpxd dev applies instances.* and onSecurityEvent from config", () => {
  it("forwards onSecurityEvent: a cross-origin control-plane POST fires origin-rejected", async () => {

...
description: Organise app logic like Phoenix contexts — routes are the thin edge, domain/ is the core, and only domain/ touches the database.
sidebar:
  order: 5
---
...
The framework never touches your database; this is purely how you organise
userland modules. Adopt it, adapt it, or ignore it.
...
> **Routes never import `db` directly. They call `domain/` functions. Only
> `domain/` imports `db`.**
...
- **Thin edge, fat core.** The handler validates (`input`), calls a domain fn,
  ...
  lives in `domain/`.
- **Swappable persistence.** Swap the client in `adapters/db.ts` and nothing
  under `routes/` changes.
- **Tests without the harness.** The pure parts (e.g. `scopeFrom`) unit-test
  with no `live()`, ctx, or db; the DB-backed queries are integration-tested
  end to end. Mock at the domain boundary — coarse and stable — instead of
  reaching for a db handle on `ctx`.
- **Transactions land where they belong.** A DB transaction opens and closes
  *inside* a domain function, exactly like `Repo.transaction` inside a Phoenix
  context. Keeping the db off `ctx` follows from this: a transaction on `ctx`
  would span a handler's awaits, and since awaits don't block the instance it
  would hold a connection open across unrelated rpcs.
...
```ts
// domain/todos.ts — the core (only this layer touches the db)
...
domain functions scope their queries by it. A bare `sid` today extends to
`{ user, org }` tomorrow without rewriting every signature. Keeping this a plain
value passed *into* domain functions — rather than the db on `ctx` — is why two
sessions never see each other's data from one shared database.

## Naming

Don't call the directory `contexts/` — rpxd already uses "context" for the
handler `ctx`, and `lib/` is conventionally shared UI. `domain/` is
collision-free; `app/` is the other reasonable choice if you want the literal
Phoenix homage.

...
- Instances are **per-stream** (one live object per tab, ADR 0003); multiplayer — and a user's other tabs — sync via pubsub (no shared instances, no `key`/`scope`)
...
Per-stream instances coordinated by broadcast (ADR 0003 — other tabs are peers like other users); persistence layer carries the bus.
